Shader "Custom/Box Outline"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                if(!any(float2(i[0].cull.x, i[1].cull.x)))
                {
                    ...
                    float4 outerColour = _OuterOutlineColour1 + outerColourVector
                                        * ((sin(_Tau * _OuterColourFreq * _Time.y) + 1) / 2.0); 

                    
                    float cornerRad = clamp(_CornerRadius, 0.0001, _OutlineThickness);
                    ...
                    float2 innerCorner0 = i[0].position.xy 
                                            + (_OutlineThickness - cornerRad) * _DistMult * i[0].smoothNormal;
                    float2 innerCorner1 = i[1].position.xy 
                                            + (_OutlineThickness - cornerRad) * _DistMult * i[1].smoothNormal;

                    float2 outerCorner0 = i[0].position.xy + _OutlineThickness * _DistMult * i[0].smoothNormal;
                    float2 outerCorner1 = i[1].position.xy + _OutlineThickness * _DistMult * i[1].smoothNormal;
